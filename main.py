...
async def cancelar(update: Update, context: ContextTypes.DEFAULT_TYPE): if block_private(update): return
...
for s in sessions.values():
    if s.get("owner_id") == user_id and s.get("running"):
        s["cancelled"] = True
        cancelled_any = True

if cancelled_any:
    await update.effective_message.reply_text("🛑 Seus downloads foram cancelados.")
else:
    await update.effective_message.reply_text("❌ Você não possui downloads ativos.")

================= START =================

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): if block_private(update): return await update.effective_message.reply_text("❌ Use o bot em grupos.") await update.effective_message.reply_text("📚 Manga Bot Online!\nUse:\n/buscar2 nome_do_manga")
...
await status.delete()
await update.message.reply_text("✅ Download finalizado!")
session["running"] = False
return ConversationHandler.END
